  ...
  implements OnInit
{
  ...
  refreshPaymentData(): void {
    this._PaymentService
      .getEduPaymentData(this.studentId)
      .subscribe((data: any) => {
        ...
        const hasFullPayment = data.educationalPayments.some(
          (p) => p.isFullPayment
        );
        paymentDetails.paymentMode = hasFullPayment ? "full" : "installments";

        const fullPayment = data.educationalPayments.find(
          (p) => p.isFullPayment
        );
        if (fullPayment) {
          ...
          paymentDetails.PaymentStatus = this.mapPaymentStatus(
            fullPayment.paymentStatus
          );
  ...
  getSelectedInstallmentAmount(): number {
    const inst = this.PaymentDetails.installments.find(
      (i) => i.id === this.selectedInstallmentId
    );
    return inst ? inst.amount + this.PaymentDetails.student.previousAmount : 0;
  }

  payFull(): void {
    const amount = this.isEligibleForDiscount
      ? this.PaymentDetails.discountedAmount
      : this.PaymentDetails.fullAmount;
  ...
  private isInstallmentsPayment(details: PaymentDetails) {
    this.isInstallmentsPaid = details.installments.some(
      (x) => x.status === "Paid" || x.status === "Pending"
    );
  }
  payBus(): void {
    const selectedBus = this.PaymentDetails.busLines.find(
      (x) => x.id === this.PaymentDetails.selectedBusLineId
    );
  ...
  isBusLinePaymentDisabled(): boolean {
    ...
    const selected = this.PaymentDetails.busLines.find(
      (x) => x.id === selectedId
    );
...
export class PaymentDetails {
  student!: StudentDTO;
  paymentMode: "full" | "installments" = "installments";
  fullAmount!: number;
