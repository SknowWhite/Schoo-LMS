...
import { CreateStudentDTO, StudentImportServiceProxy, StudentDTO, IStudentDto } from '@shared/service-proxies/service-proxies';
...
export class StudentsComponent extends AppComponentBase implements OnInit {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        if (sheetNames.includes('STD')) {
          ...
          const rawStudentsData = XLSX.utils.sheet_to_json(studentsSheet, {defval: ''});
        ...
        if (sheetNames.includes('Educational Fees')) {
          ...
          const rawEducationalFeesData = XLSX.utils.sheet_to_json(educationalFeesSheet, {defval: ''});
          if (rawEducationalFeesData.length > 0) {
            ...
            if (this.hasEducationalFeesData && !this.hasStudentsData && !this.activeTab) {
              this.activeTab = 'educationalFees';
            }
          }
        } else {
          this.validationErrors.push(this.l('SheetNotFound', 'Educational Fees'));
        }
        
        // Process Bus Fees sheet
        if (sheetNames.includes('Bus Fees')) {
          const busFeesSheet = workBook.Sheets['Bus Fees'];
          const rawBusFeesData = XLSX.utils.sheet_to_json(busFeesSheet, {defval: ''});
          if (rawBusFeesData.length > 0) {
            ...
            if (this.hasBusFeesData && !this.hasStudentsData && !this.hasEducationalFeesData && !this.activeTab) {
        ...
        if (!this.hasStudentsData && !this.hasEducationalFeesData && !this.hasBusFeesData) {
  ...
  processStudentsData(rawData: any[]): void {
    ...
    const requiredColumns = ['Name', 'Grade', 'Previous Amount', 'Mobile Number', 'Status_Des','id'];
  ...
  processEducationalFeesData(rawData: any[]): void {
    ...
    const requiredColumns = [
      'Grade', 'Expected Amount', 'First Installment Before 1-6', 
      'Second Installment Before 1-8', 'Third Installment Before 1-10', 
      'Fourth Installment Before 1-12', 'Full Amount Before 1-6'
    ];
  ...
  processBusFeesData(rawData: any[]): void {
    // Define required columns
    const requiredColumns = [
      'Lines', 'Expected Amount', 'First Installment Before 1-6', 
      'Second Installment Before 1-8', 'Third Installment Before 1-10', 
      'Fourth Installment Before 1-12', 'Full Amount Before 1-6'
    ];
  ...
  validateEducationalFeeData(fee: EducationalFeeDTO, index: number): void {
    ...
    if (fee.expectedAmount <= 0) {
      this.validationErrors.push(this.l('InvalidExpectedAmount', 'Educational Fee', index + 1));
    }
    
    // Validate installment amounts
    if (fee.firstInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'First', 'Educational Fee', index + 1));
    }
    if (fee.secondInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Second', 'Educational Fee', index + 1));
    }
    if (fee.thirdInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Third', 'Educational Fee', index + 1));
    }
    if (fee.fourthInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Fourth', 'Educational Fee', index + 1));
    }
    if (fee.fullAmountWithDiscount.amount < 0) {
      this.validationErrors.push(this.l('InvalidFullAmountWithDiscount', 'Educational Fee', index + 1));
    }
  }
  ...
  validateBusFeeData(fee: BusFeeDTO, index: number): void {
    ...
    if (fee.expectedAmount <= 0) {
      this.validationErrors.push(this.l('InvalidExpectedAmount', 'Bus Fee', index + 1));
    }
    
    // Validate installment amounts
    if (fee.firstInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'First', 'Bus Fee', index + 1));
    }
    if (fee.secondInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Second', 'Bus Fee', index + 1));
    }
    if (fee.thirdInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Third', 'Bus Fee', index + 1));
    }
    if (fee.fourthInstallment.amount < 0) {
      this.validationErrors.push(this.l('InvalidInstallmentAmount', 'Fourth', 'Bus Fee', index + 1));
    }
    if (fee.fullAmountWithDiscount.amount < 0) {
      this.validationErrors.push(this.l('InvalidFullAmountWithDiscount', 'Bus Fee', index + 1));
    }
  }
  
  // Helper methods for data extraction and validation
  
  checkRequiredColumns(data: any[], requiredColumns: string[], sheetName: string): void {
    if (data.length === 0) {
      return;
    }
    ...
    for (const column of requiredColumns) {
      if (firstRow[column] === undefined) {
        this.validationErrors.push(this.l('ColumnNotFoundInSheet', column, sheetName));
      }
    }
  }
  ...
  saveData(): void {
    ...
    // Simulate successful save for now
      ...
      this._importService.ImportEducationalFees(this.educationalFeesData).subscribe((result) => {
      ...
      this._importService.ImportStudents(this.studentsData).subscribe((result) => {
        this.notify.success(this.l('DataSavedSuccessfully'));
      });
  }
  ...
  getEducationalFees(){
    this._importService.getAllEducationsFees('',0,1000000).subscribe((data)=>{
